...
from wrapper import get_instrumented_model
...
def load_components(class_name, inst):
    ...
    components = SimpleNamespace(
        X_comp = jnp.array(X_comp)
        X_mean = jnp.array(X_mean)
        X_stdev = jnp.array(X_stdev)
        Z_comp = jnp.array(Z_comp)
        Z_stdev = jnp.array(Z_stdev)
        Z_mean = jnp.array(Z_mean)
...
def load_named_components(path, class_name):
    ...
    for d in selected:
        ...
        components.X_comp.append(jax.array(d['act_comp']))
        components.Z_comp.append(jax.array(d['lat_comp']))
...
def setup_model():
    ...
    # Load model
    inst = get_instrumented_model(model_name, class_name, layer_name, None, use_w=args.use_w)
    model = inst.model
