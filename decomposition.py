...
def load_network(out_class, model=2,addr=None):
...
def pca(G, stylegan_version, out_class, estimator='ipca', batch_size=20, num_components=80, num_samples=1_000_000, use_w=True, force_recompute=False, seed_compute=None):
    ...
    if not dump_path.is_file() or force_recompute:
        ...
        compute_pca(G, estimator, batch_size, num_components, num_samples, use_w, seed_compute, dump_path)

    return dump_path


def compute_pca(G, estimator, batch_size, num_components, num_samples, use_w, seed, dump_path):
    global B
    ...
    latent_in = random.normal(rng, (1, 512))
    latent = G.apply_mapping({'params': G.params['mapping'], 'moving_stats': G.moving_stats},latent_in,train=False)
    sample_shape = latent.shape
    sample_dims = np.prod(sample_shape)
    print("Feature shape: ", sample_shape)
    print("Feature dims: ", sample_dims)

    input_shape = (1,15, 512)
    ...
    for i in trange(n_lat // B, desc='Sampling latents'):
        ...
        if use_w:
            w = G.apply_mapping({'params': G.params['mapping'], 'moving_stats': G.moving_stats},z,train=False)
            # print(w.shape)
            latents[i*B:(i+1)*B] = w
        else:
            latents[i*B:(i+1)*B] = z
    ...
    if use_w:
        ...
        samples =G.apply_mapping({'params': G.params['mapping'], 'moving_stats': G.moving_stats},z,train=False).reshape(5000, input_dims)
