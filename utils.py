...
def centre_strip_stylegan(G, z, lat_comp, lat_mean, lat_stdev, idx, sigma, num_frames, layer_start, layer_end,w_avg,truncation_psi):
        ...
        for j in range(len(sigma_range)):
            ...
            ws = G.apply_mapping({'params': G.params['mapping'], 'moving_stats': G.moving_stats},z,train=False)
            # ws = ws.reshape((num_layers, 1, input_shape[-1]))

            s = sigma_range[j]

            delta = lat_comp[idx] * s * lat_stdev[idx]
            # print(delta.shape,zeroing_offset_lat.shape,ws.shape)
            # for k in range(layer_start, layer_end):
                # ws[k] = ws[k] - zeroing_offset_lat + delta
            ws = ws - zeroing_offset_lat + delta
            ws = w_avg + (ws - w_avg) * truncation_psi
            # imgs = Gs.components.synthesis.run(ws.reshape((1, num_layers, input_shape)), **Gs_kwargs)
            imgs=G.apply_synthesis({'params': G.params['synthesis'], 'moving_stats': G.moving_stats,'noise_consts': G.noise_consts},ws.reshape((1, num_layers, input_shape[-1])), noise_mode='none')
